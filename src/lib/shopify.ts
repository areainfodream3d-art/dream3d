...
export const normalizeShopifyAssetUrl = (rawUrl: string | undefined | null) => {
  if (!rawUrl) return rawUrl as any;

  const base = `https://${domain}`;

  try {
    const u = new URL(rawUrl, base);
    if (u.pathname.startsWith('/cdn/shop/')) {
      u.protocol = 'https:';
      u.host = domain;
    }
    return u.toString();
  } catch {
    if (typeof rawUrl === 'string' && rawUrl.startsWith('/cdn/shop/')) {
      return `${base}${rawUrl}`;
    }
