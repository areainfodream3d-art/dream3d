...
import { addToCart as addToShopifyCart } from '../lib/shopifyCart';
...
export const ProductDetails = () => {
  ...
  const normalizeCollections = (p: any): { title: string; handle: string }[] => {
  ...
  const uniqueCollections = (p: any): { title: string; handle: string }[] => {
    const list = normalizeCollections(p);
    const map = new Map<string, { title: string; handle: string }>();
    for (const c of list) {
      const title = String(c.title || '').trim();
      const handle = String(c.handle || '').trim();
      const key = (title || handle).toLowerCase();
      if (!key) continue;
      if (!map.has(key)) map.set(key, { title, handle });
    }
  ...
  const addToCart = async () => {
    ...
    if (!selectedVariant.id || typeof selectedVariant.id !== 'string') {
      alert('Prodotto non acquistabile: variante non disponibile.');
      return;
    }
    
    setAdding(true);
    try {
      const checkout = await addToShopifyCart(selectedVariant.id, 1);
    ...
    <div className="pt-32 pb-20 container mx-auto px-4 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          {uniqueCollections(product).length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              {uniqueCollections(product).slice(0, 6).map((c) => (
                <span
                  key={c.handle || c.title}
                  className="text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full bg-black/40 text-gray-300 border border-white/10"
                >
                  {c.title || c.handle}
                </span>
              ))}
