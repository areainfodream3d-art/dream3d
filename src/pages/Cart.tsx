...
export const Cart = () => {
    ...
    if (checkoutId) {
      try {
        ...
        if (!checkout || checkout.completedAt || !checkout.webUrl) {
          ...
          setCart(null);
          window.dispatchEvent(new CustomEvent('dream3d:cart-updated', { detail: { checkout: null } }));
        } else {
          setCart(checkout);
          window.dispatchEvent(new CustomEvent('dream3d:cart-updated', { detail: { checkout } }));
        }
      } catch (error) {
  ...
  const removeItem = async (lineItemId: string) => {
    ...
    if (checkoutId) {
      ...
      const updated = await shopify.checkout.removeLineItems(checkoutId, [lineItemId]);
      window.dispatchEvent(new CustomEvent('dream3d:cart-updated', { detail: { checkout: updated } }));
      await fetchCart();
    }
    setIsUpdating(false);
  };
  ...
  const proceedToCheckout = () => {
    if (!isShopifyConfigured) {
      alert('Acquisto non disponibile: Shopify non configurato.');
      return;
    }
    if (!isLoggedIn) {
      const goLogin = window.confirm('Vuoi accedere prima di pagare? OK = Accedi, Annulla = Continua come ospite.');
      if (goLogin) {
        navigate('/account');
        return;
      }
    ...
    if (cart && cart.webUrl) {
      try {
        localStorage.setItem('lastCheckoutWebUrl', cart.webUrl);
        localStorage.setItem('lastCheckoutAt', new Date().toISOString());
      } catch {}
      window.location.href = cart.webUrl;
      return;
    }
    alert('Checkout Shopify non disponibile.');
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-1">
          <div className="bg-dark-surface p-6 rounded-2xl border border-white/10 sticky top-32">
            ...
            {!isLoggedIn && (
              <div className="mb-6 p-4 rounded-xl border border-white/10 bg-dark-bg">
                <div className="text-sm text-gray-300 font-bold mb-2">Stai acquistando come ospite</div>
                <div className="text-xs text-gray-500 mb-3">Puoi accedere per associare l’ordine al tuo account, oppure continuare senza login.</div>
                <Button variant="outline" size="sm" onClick={() => navigate('/account')}>Accedi</Button>
              </div>
            )}
