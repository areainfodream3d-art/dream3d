...
import { normalizeShopifyAssetUrl } from '../../lib/shopify';
...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const collectionsRaw =
    product.collections && product.collections.edges
      ? product.collections.edges.map((e: any) => e.node).filter(Boolean)
  ...
  const collections = React.useMemo(() => {
    ...
    for (const c of collectionsRaw) {
      ...
      if (!key) continue;
      if (!map.has(key)) map.set(key, { ...c, title, handle });
    }
  ...
  if (media && media.length > 0) {
    ...
    if (bestGlb) {
        has3DModel = true;
        modelUrl = normalizeShopifyAssetUrl(bestGlb.url);
    } else if (anyGlb) {
        // Fallback to ANY GLB found if no "texture" one exists
        has3DModel = true;
        modelUrl = normalizeShopifyAssetUrl(anyGlb.url);
    }
    ...
    if (!has3DModel) {
        for (const m of media) {
            if (m.sources) {
                ...
                if (stlSource) {
                    has3DModel = true;
                    modelUrl = normalizeShopifyAssetUrl(stlSource.url);
                    break;
                }
            }
        }
    ...
    if (!has3DModel) {
        ...
        if (modelMedia && modelMedia.sources && modelMedia.sources.length > 0) {
            ...
            if (source) {
                has3DModel = true;
                modelUrl = normalizeShopifyAssetUrl(source.url);
            }
        } else {
            ...
            for (const m of media) {
                if (m.sources) {
                    ...
                    if (fallbackSource) {
                        has3DModel = true;
                        modelUrl = normalizeShopifyAssetUrl(fallbackSource.url);
                        break;
                    }
                }
            }
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {collections.slice(0, 3).map((c: any) => (
                <span
                  ...
                  className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full bg-black/40 text-gray-300 border border-white/10"
                >
                  {c.title || c.handle}
                </span>
              ))}
