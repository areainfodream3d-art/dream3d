...
import { ConnectionAvatar } from '../ui/ConnectionAvatar';
...
export const Navbar = () => {
  ...
  const { isLoggedIn, loading } = useShopifyCustomer();
      ...
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="flex md:order-2 space-x-3 md:space-x-4 rtl:space-x-reverse items-center">
          ...
          <Link to="/carrello" className="relative text-gray-300 hover:text-neon-orange transition-colors">
            ...
            <span className="absolute -top-2 -right-2 bg-neon-fire text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{count}</span>
          </Link>
          <Link to={!loading && isLoggedIn ? "/ordini" : "/account"} className="text-gray-300 hover:text-neon-orange transition-colors">
            <ConnectionAvatar connected={!loading && isLoggedIn} size="sm" />
          </Link>
          <Link to="/commissioni#preventivo" className="hidden md:flex">
